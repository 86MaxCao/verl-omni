#!/usr/bin/env bash
# FlowGRPO Bagel t2i (text-to-image) e2e smoke test.
#
# Exercises the full Bagel FlowGRPO RL pipeline:
#   parquet load -> vllm_omni rollout (BagelPipelineWithLogProb) ->
#   visual reward (jpeg_compressibility) -> flow_grpo -> FSDP LoRA -> sync.
#
...
t2i_dir=${DATA_DIR}/t2i
dummy_train_path=${TRAIN_FILES:-${t2i_dir}/train.parquet}
dummy_test_path=${VAL_FILES:-${t2i_dir}/test.parquet}
...
python3 -m verl_omni.trainer.main_diffusion \
    ...
    trainer.experiment_name=flowgrpo-bagel-t2i-e2e \
...
echo "FlowGRPO Bagel t2i e2e test passed (training completed successfully)."
